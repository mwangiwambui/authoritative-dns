pipeline {
    ...
    stage("Build image") {
        ...
        stage('Deploy to GKE') {
            steps{
                sh '''
                sed -i "s/us-central1-docker.pkg.dev/devops-capstone-project-395319\/dns-authoritative\/dns-authoritative:latest\/us-central1-docker.pkg.dev\/devops-capstone-project-395319\/dns-authoritative/dns-authoritative:latest/g" deployment.yml
                '''
